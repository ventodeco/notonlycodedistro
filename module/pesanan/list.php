<?php
	...
	if(mysqli_num_rows($queryPesanan) == 0){
		...
		while($row=mysqli_fetch_assoc($queryPesanan)){
			if($level == "superadmin"){
				$adminButton = "<a class='tombol-action' href='".BASE_URL."index.php?page=profilku&module=pesanan&action=status&pesanan_id=$row[pesanan_id]'>Update Status</a>";
			}
			...
			echo "<tr>
					...
					<td class='kiri'>
						<a class='tombol-action' href='".BASE_URL."index.php?page=profilku&module=pesanan&action=detail&pesanan_id=$row[pesanan_id]'>Detail Pesanan</a>
						$adminButton
					</td>
				 </tr>";
		}
